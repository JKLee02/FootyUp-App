...
app.post('/signup', async (req, res) => {
    ...
    db.query('SELECT * FROM user WHERE email = ?', [email], (err, result) => {
        ...
        db.query('INSERT INTO user (`user_firstname`, `user_lastname`, `email`, `password`) VALUES (?, ?, ?, ?)', 
...
app.post('/login', (req, res) => {
    ...
    db.query('SELECT * FROM user WHERE email = ?', [email], async (err, result) => {
        ...
        const accessToken = jwt.sign({ id: user.user_id }, jwtSecret, { expiresIn: '1h' });
        const refreshToken = jwt.sign({ id: user.user_id }, jwtSecret, { expiresIn: '7d' });
...
app.get('/uservenue', (req, res) => {
    ...
    db.query(query, (err, result) => {
        if (err) return res.status(500).json({ message: 'Error retrieving venues' });
...
app.get('/tournaments', (req, res) => {
  ...
  db.query(query, (err, result) => {
      if (err) return res.status(500).json({ message: 'Error retrieving tournaments' });
      res.json(result); // Send venues as JSON
  });
});
...
app.get('/tournaments/pendingapproval', (req, res) => {
  const query = 'SELECT * FROM tournament WHERE tournament_status = "Pending Approval"';
  db.query(query, (err, result) => {
    if (err) {
...
app.get('/matches', (req, res) => {
  ...
  db.query(query, (err, result) => {
      if (err) return res.status(500).json({ message: 'Error retrieving matches' });
      res.json(result); // Send venues as JSON
  });
});
...
app.get('/matches/completed', (req, res) => {
  ...
  db.query(query, (err, result) => {
      if (err) return res.status(500).json({ message: 'Error retrieving completed matches' });
      res.json(result); // Send venues as JSON
  });
});
...
app.get('/uservenue/:id', (req, res) => {
    ...
    db.query(query, [venueId], (err, result) => {
        if (err) return res.status(500).json({ message: 'Error retrieving venue details' });
        if (result.length === 0) return res.status(404).json({ message: 'Venue not found' });
        res.json(result[0]);
    });
});
...
app.post('/venue', (req, res) => {
    const { venue_name, venue_address, venue_description, venue_image } = req.body;
    const query = 'INSERT INTO venue (venue_name, venue_address, venue_description, venue_image) VALUES (?, ?, ?, ?)';
    db.query(query, [venue_name, venue_address, venue_description, venue_image], (err, result) => {
        if (err) {
...
app.put('/venue/:id', (req, res) => {
    ...
    const query = 'UPDATE venue SET venue_name = ?, venue_address = ?, venue_description = ?, venue_image = ? WHERE venue_id = ?';
    db.query(query, [venue_name, venue_address, venue_description, venue_image, venueId], (err, result) => {
        if (err) {
...
app.get('/userprofile', (req, res) => {
    try {
        ...
        db.query('SELECT user_firstname, user_lastname, email, password, user_gender FROM user WHERE user_id = ?', [decoded.id], (err, result) => {
            if (err) {
...
app.post('/teams', (req, res) => {
  const { name, description, membersAmount, teamImage, captainId, captainName } = req.body;
...
db.query(createTeamQuery, [name, teamDescription || null, teamImage || null, captainId, captainName], (err, result) => {
  if (err) {
...
app.put('/teams/:teamId', (req, res) => {
  ...
  const checkCaptainQuery = 'SELECT * FROM team WHERE team_id = ? AND team_captain_id = ?';
  db.query(checkCaptainQuery, [teamId, userId], (err, teamResult) => {
    ...
    const updateTeamQuery = `
      ...
      WHERE team_id = ?`;
    db.query(updateTeamQuery, [name, description || null, teamImage || null, teamId], (err) => {
      if (err) {
...
app.post('/teams/:teamId/members', (req, res) => {
    ...
    db.query(checkTeamQuery, [teamId], (err, teamResult) => {
      ...
      db.query(checkUserTeamQuery, [userId], (err, userResult) => {
        ...
        const updateUserTeamQuery = 'UPDATE user SET user_team = ? WHERE user_id = ?';
...
app.post('/teams/:teamId/leave', (req, res) => {
  ...
  db.query(checkTeamQuery, [teamId], (err, teamResult) => {
    ...
    const checkUserTeamQuery = 'SELECT * FROM user WHERE user_id = ? AND user_team = ?';
    db.query(checkUserTeamQuery, [userId, teamId], (err, userResult) => {
      ...
      const updateUserTeamQuery = 'UPDATE user SET user_team = NULL WHERE user_id = ?';
...
app.delete('/teams/:teamId/delete', (req, res) => {
  ...
  db.query(checkCaptainQuery, [teamId, userId], (err, teamResult) => {
    ...
    if (teamResult.length === 0) {
      return res.status(404).json({ error: 'Team not found or user is not the captain' });
    }

    // Remove all users from the team
    const removeUsersQuery = 'UPDATE user SET user_team = NULL WHERE user_team = ?';
...
app.post('/tournaments', (req, res) => {
  const { title, description, startDate, startTime, gender, venueId, userId } = req.body;

  // Check if the user is in a team
  const checkUserTeamQuery = 'SELECT * FROM user WHERE user_id = ? AND user_team IS NOT NULL';
  db.query(checkUserTeamQuery, [userId], (err, userResult) => {
    if (err) {
      console.error('Error checking user team:', err);
      return res.status(500).json({ error: 'Database error' });
    ...
    if (userResult.length === 0) {
      return res.status(403).json({ error: 'User is not in a team, please join a team to create one' });
    }
    ...
    db.query(checkVenueQuery, [venueId], (err, venueResult) => {
      ...
      `;
      db.query(createTournamentQuery, [
        title, description, startDate, startTime, gender, venue.venue_id, venue.venue_name, venue.venue_description, 
        venue.venue_image, user.user_id, user.user_firstname
      ], (err, result) => {
        if (err) {
...
app.get('/tournaments/created/:userId', (req, res) => {
  ...
  const query = 'SELECT * FROM tournament WHERE user_created_id = ? AND tournament_status = "Approved"';
...
app.delete('/tournaments/:tournamentId', (req, res) => {
  ...
  const deleteMatchesQuery = 'DELETE FROM matches WHERE match_tournament_id = ?';
  db.query(deleteMatchesQuery, [tournamentId], (err) => {
    if (err) {
      console.error('Error deleting tournament matches:', err);
      return res.status(500).json({ error: 'Failed to delete tournament matches' });
    }

    // Then, delete the tournament itself
    const deleteTournamentQuery = 'DELETE FROM tournament WHERE tournament_id = ?';
    db.query(deleteTournamentQuery, [tournamentId], (err, result) => {
      ...
      res.json({ message: 'Tournament and its associated matches deleted successfully' });
    });
  });
});
...
app.put('/tournaments/:tournamentId', (req, res) => {
  const { tournamentId } = req.params;
  const { tournament_title, tournament_description, tournament_date, tournament_time, players_gender, tournament_status } = req.body;
  ...
  db.query(query, [tournament_title, tournament_description, tournament_date, tournament_time, players_gender, tournament_status, tournamentId], (err, result) => {
    if (err) {
...
app.post('/tournaments/:tournamentId/join', (req, res) => {
  ...
  db.query(checkTeamCountQuery, [tournamentId], (err, results) => {
    ...
    if (results[0].total_teams >= teamLimit) {
      return res.status(400).json({ error: `Cannot join this tournament. Maximum number of teams (${teamLimit}) reached.` });
    }
    ...
    db.query(checkUserTeamQuery, [userId], (err, userResult) => {
      ...
      db.query(fetchTeamNameQuery, [teamId], (err, teamResult) => {
        ...
        const checkTeamInTournamentQuery = 'SELECT * FROM matches WHERE match_tournament_id = ? AND (team_1_id = ? OR team_2_id = ?)';
        db.query(checkTeamInTournamentQuery, [tournamentId, teamId, teamId], (err, matchResult) => {
          if (err) {
          ...
          if (matchResult.length > 0) {
            return res.status(403).json({ error: 'Team is already in the tournament' });
          }
          ...
          db.query(findEmptySlotQuery, [tournamentId], (err, emptySlotResult) => {
            if (err) {
            ...
            if (emptySlotResult.length > 0) {
              ...
              db.query(updateTeam1Query, [teamId, teamName, match.match_id], (err) => {
                if (err) {
                ...
                db.query(updateTeam2Query, [teamId, teamName, match.match_id], (err) => {
                  if (err) {
                  ...
                  `;
                  db.query(checkMatchFullQuery, [match.match_id], (err, fullMatchResult) => {
                    if (err) {
                    ...
                    if (fullMatchResult.length > 0) {
                      console.log('Match is now full and ready to play.');
                    }
              ...
              const insertMatchQuery = `
                ...
                VALUES (?, ?, ?, 1, 'Scheduled')
              `;
              db.query(insertMatchQuery, [teamId, teamName, tournamentId], (err, result) => {
                if (err) {
...
app.get('/tournaments/:tournamentId/team/:teamId', (req, res) => {
  ...
  const query = 'SELECT * FROM matches WHERE match_tournament_id = ? AND (team_1_id = ? OR team_2_id = ?)';
...
app.post('/tournaments/:tournamentId/leave', (req, res) => {
  ...
  db.query(checkUserTeamQuery, [userId], (err, userResult) => {
    ...
    db.query(checkCompletedMatchesQuery, [tournamentId, teamId, teamId], (err, matchResult) => {
      if (err) {
      ...
      const hasCompletedMatch = matchResult.some(match => match.match_status === 'Completed');

      // Check if the team is in the finals
      const isInFinals = matchResult.some(match => match.match_round === 'Final');

      if (hasCompletedMatch || isInFinals) {
        return res.status(403).json({ error: 'Cannot leave. Team has completed matches or is in the finals.' });
      }
...
app.put('/matches/:matchId/scores', (req, res) => {
  ...
  db.query(checkTournamentCreatorQuery, [matchId, userId], (err, result) => {
    ...
    if (result.length === 0) {
      return res.status(403).json({ error: 'User is not the tournament creator' });
    }
    ...
    db.query(checkSemiFinalsQuery, [tournamentId], (err, semiFinalResult) => {
      ...
      if (matchRound === 1 && completedSemiFinals === 2) {
        return res.status(400).json({ error: 'Cannot edit semi-final matches after completion.' });
      }
      ...
      const updateScoresQuery = `
        ...
        WHERE match_id = ?
      `;
      db.query(updateScoresQuery, [score_team_1, score_team_2, matchId], (err) => {
        if (err) {
        ...
        `;
        db.query(checkFinalExistsQuery, [tournamentId], (err, finalCountResult) => {
          if (err) {
          ...
          `;
          db.query(checkSemiFinalsQuery, [tournamentId], (err, semiFinalsResult) => {
            if (err) {
            ...
            if (semiFinalsResult.length === 2) {
              const insertFinalMatchQuery = `
                ...
                VALUES (?, ?, ?, ?, ?, 2, 'Scheduled')
              `;
              db.query(insertFinalMatchQuery, [
                semiFinalsResult[0].winner_team_id, semiFinalsResult[0].winner_team_name,
                semiFinalsResult[1].winner_team_id, semiFinalsResult[1].winner_team_name,
                tournamentId
              ], (err) => {
                if (err) {
    ...
    if (results[0].incomplete_matches === 0) {
      ...
      db.query(updateTournamentStatusQuery, [tournamentId], (err) => {
        if (err) {
          ...
          console.log(`Tournament ${tournamentId} status updated to 'Completed'`);
        }
      });
    }
  });
...
app.post('/adminlogin', (req, res) => {
    ...
    db.query('SELECT * FROM admin WHERE admin_email = ?', [email], async (err, result) => {
        ...
        const accessToken = jwt.sign({ id: admin.admin_id }, jwtSecret, { expiresIn: '1h' });
        const refreshToken = jwt.sign({ id: admin.admin_id }, jwtSecret, { expiresIn: '7d' });
  ...
  app.put('/tournaments/:tournamentId/approve', (req, res) => {
    ...
    const query = 'UPDATE tournament SET tournament_status = "Approved" WHERE tournament_id = ?';
    db.query(query, [tournamentId], (err, result) => {
      if (err) {
...
app.put('/tournaments/:tournamentId/deny', (req, res) => {
  ...
  const query = 'UPDATE tournament SET tournament_status = "Denied" WHERE tournament_id = ?';
  db.query(query, [tournamentId], (err, result) => {
    if (err) {
