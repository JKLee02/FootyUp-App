...
function UserTournamentDetails() {
    ...
    const fetchTournamentAndMatchData = async () => {
      try {
        ...
        const userResponse = await fetch(
          `http://localhost:8081/user/${userId}/team`,
        );
        if (!userResponse.ok) {
  ...
  const handleJoinTournament = async () => {
    try {
      ...
      const teamCountResponse = await fetch(
        `http://localhost:8081/tournaments/${tournamentId}/teams/count`,
      );
      ...
      if (teamCountData.count >= teamLimit) {
        alert(
          `Cannot join this tournament. Maximum number of teams (${teamLimit}) reached.`,
        );
        return;
      }
      ...
      const response = await fetch(
        `http://localhost:8081/tournaments/${tournamentId}/join`,
        {
          method: "POST",
  ...
  const handleLeaveTournament = async () => {
    try {
      const response = await fetch(
        `http://localhost:8081/tournaments/${tournamentId}/leave`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ userId }),
  ...
  return (
    <div className="user-tournament-details-page">
      ...
      <main className="user-tournament-details-content">
        <h1 className="user-tournament-title">
        ...
        <div className="user-tournament-details-layout">
          <div className="user-tournament-info">
            ...
            <p>
              <strong>Gender:</strong> {tournamentData.players_gender}
            </p>
            <p>
              <strong>Created by:</strong> {tournamentData.user_created_name}
            </p>
            <div className="user-tournament-note">
              Note: Tournaments are currently in single elimination format with
              only 4 teams and only 5-a-side teams are allowed to join. Only
              players in teams can join.
            </div>
          ...
          <div className="user-tournament-venue">
            ...
            <h2 className="user-tournament-venue-name">
              {venueData.venue_name}
            </h2>
            <p className="user-tournament-venue-address">
