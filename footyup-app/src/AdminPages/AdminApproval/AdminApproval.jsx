...
function AdminApproval() {
  ...
  const handleApprove = async (row) => {
    try {
      const response = await fetch(
        `http://localhost:8081/tournaments/${row.tournament_id}/approve`,
        {
          method: "PUT",
      ...
      setTournaments(
        tournaments.map((t) =>
          t.tournament_id === row.tournament_id
  ...
  const handleDeny = async (row) => {
    try {
      const response = await fetch(
        `http://localhost:8081/tournaments/${row.tournament_id}/deny`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
        },
  ...
  const filteredTournaments = tournaments.filter(
    (tournament) => tournament.tournament_status === "Pending Approval",
  );
